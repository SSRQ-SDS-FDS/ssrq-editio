...
<xsl:stylesheet xmlns:cutils="http://ssrq-sds-fds.ch/xsl/tei2pub/functions/cutils"
    ...
    <xsl:template match="/">
        <xsl:variable name="idno" as="text()" select=".//tei:seriesStmt/tei:idno[not(@type)]/text()"/>
        <xsl:map>
            <xsl:map-entry key="'uuid'" select=".//tei:idno[@type = 'uuid']/text()"/>
            ...
            <xsl:map-entry key="'de_title'" select="()"/>
            <xsl:map-entry key="'fr_title'" select="()"/>
            ...
            <xsl:map-entry key="'orig_place'">
                <xsl:choose>
                    <xsl:when test=".[tei:origPlace[@type='document']][tei:origPlace[@type='content']]">
                        <xsl:value-of select="./tei:origPlace[@type='content']/@ref"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="./tei:origPlace/@ref"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:map-entry>
        </xsl:if>
