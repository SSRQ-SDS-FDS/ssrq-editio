...
def load_and_process(page: Page, html: str) -> str:
    ...
    source = Path("src/ssrq_editio/entrypoints/app/static/js/components/spans.js").read_text()
    page.add_script_tag(
        type="module",
...
def test_process_span_markers_registers_ranges(page: Page) -> None:
    ...
    result = page.evaluate(
        """() => { const h = CSS.highlights.get("tei-additions"); return { rangeCount: h.size, text: [...h][0].toString() }; }"""
    )
...
def test_process_span_markers_reports_missing_end_marker(page: Page) -> None:
    ...
    load_and_process(
        page, """<div><span class="addSpanStart" data-addspan-id="add1">​</span><p>foo</p></div>"""
    )
