    ...
    return
        element { node-name($node) } {
            ...
            return
                if ($kanton => contains('-'))
                then
                    kantons:render-merged($all-kantons($kanton), $kanton, $existing-kantons, $lang)
                else
                    kantons:render($all-kantons($kanton), $kanton, $existing-kantons, $lang)
        }
};
...
: @param $lang xs:string - the language of the current page
: @return element() - the rendered HTML-fragment
:)
declare %private function kantons:render-merged($kanton-info as map(*), $kanton as xs:string, $existing-kantons as element(docs), $lang as xs:string) as element(tr) {
            ...
            {kantons:link-to-kanton(
                <div class="kanton-card_inner-container">
                    ...
                    <div class="kanton-card_content">
                        ...
                        {kantons:render-department-title($kanton-info($kanton-parts[1]), $kanton, $lang)}
                    </div>
                </div>,
                $kanton, $kanton-docs)}
        </div>
...
: @return element() - the rendered HTML-fragment
:)
declare %private function kantons:render($kanton-info as map(*), $kanton as xs:string, $existing-kantons as element(docs), $lang as xs:string) as element(div) {
    let $kanton-docs as element(kanton)? := id($kanton, $existing-kantons)
    return
        <div class="kanton-card {if (exists($kanton-docs)) then 'has-docs' else 'inactive'}">
            {kantons:link-to-kanton(
                <div class="kanton-card_inner-container">
                    ...
                    <div class="kanton-card_content">
                        ...
                        {kantons:render-department-title($kanton-info, $kanton, $lang)}
                    </div>
                </div>,
                $kanton, $kanton-docs)}
        </div>
...
:)
declare %private function kantons:render-department-title($kanton-info as map(*), $department as xs:string, $lang as xs:string) as element(h3) {
    let $title := $kanton-info?title($lang)
